...
class PostBloc extends Bloc<PostEvent, PostState> {
    ...
    if(event is Fetch && !_hasReachedMax(currentState)){
      try{
        ...
        if (currentState is PostLoaded){
          ...
          yield posts.isEmpty ? (currentState as PostLoaded).copyWith(hasReachedMax: true) : PostLoaded(hasReachedMax: false, posts: (currentState as PostLoaded).posts + posts);
        }
      } catch (_) {
        yield PostError();
      }
  ...
  Stream<PostState> transform(Stream<PostEvent> events, Stream<PostState> Function(PostEvent event) next) {
    ...
    return super.transform((events as Observable<PostEvent>).debounce(Duration(milliseconds: 500)), next);
  }
}